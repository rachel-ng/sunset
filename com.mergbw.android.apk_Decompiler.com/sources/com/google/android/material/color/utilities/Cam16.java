...
public final class Cam16 {
    ...
    static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d15, d17, pow4, c2, flRoot, Math.sqrt((pow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * pow4) / ((0.007d * pow4) + 1.0d), log1p * Math.cos(radians), log1p * Math.sin(radians));
    }
    ...
    private static Cam16 fromJchInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        double d5 = d3;
        double d6 = d / 100.0d;
        double c2 = (4.0d / viewingConditions.getC()) * Math.sqrt(d6) * (viewingConditions.getAw() + 4.0d) * viewingConditions.getFlRoot();
        ...
        return new Cam16(d5, d4, d, c2, flRoot, Math.sqrt(((d2 / Math.sqrt(d6)) * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, d7, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
