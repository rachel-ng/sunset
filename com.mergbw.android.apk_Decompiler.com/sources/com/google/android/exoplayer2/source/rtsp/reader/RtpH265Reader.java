...
final class RtpH265Reader implements RtpPayloadReader {
    ...
    private void processFragmentationUnitPacket(ParsableByteArray parsableByteArray, int i) throws ParserException {
        if (parsableByteArray.getData().length >= 3) {
            byte b2 = parsableByteArray.getData()[1] & 7;
            byte b3 = parsableByteArray.getData()[2];
            byte b4 = b3 & Utf8.REPLACEMENT_BYTE;
            boolean z = (b3 & 128) > 0;
            boolean z2 = (b3 & SignedBytes.MAX_POWER_OF_TWO) > 0;
            if (z) {
                this.fragmentedSampleSizeBytes += writeStartCode();
                parsableByteArray.getData()[1] = (byte) ((b4 << 1) & 127);
                parsableByteArray.getData()[2] = (byte) b2;
            ...
            this.fragmentedSampleSizeBytes += bytesLeft;
            if (z2) {
                this.bufferFlags = getBufferFlagsFromNalType(b4);
                return;
            }
            return;
        }
