...
final class RtpH264Reader implements RtpPayloadReader {
    ...
    public void consume(ParsableByteArray parsableByteArray, long j, int i, boolean z) throws ParserException {
        int i2 = i;
        try {
            byte b2 = parsableByteArray.getData()[0] & Ascii.US;
            ...
            if (b2 > 0 && b2 < 24) {
                ...
                throw ParserException.createForMalformedManifest(String.format("RTP H264 packetization mode [%d] not supported.", new Object[]{Integer.valueOf(b2)}), (Throwable) null);
            }
            if (z) {
                ...
                this.trackOutput.sampleMetadata(RtpReaderUtils.toSampleTimeUs(this.startTimeOffsetUs, j, this.firstReceivedTimestamp, MEDIA_CLOCK_FREQUENCY), this.bufferFlags, this.fragmentedSampleSizeBytes, 0, (TrackOutput.CryptoData) null);
    ...
    private void processFragmentationUnitPacket(ParsableByteArray parsableByteArray, int i) {
        ...
        byte b4 = (b2 & 224) | (b3 & Ascii.US);
        boolean z = (b3 & 128) > 0;
        boolean z2 = (b3 & SignedBytes.MAX_POWER_OF_TWO) > 0;
        if (z) {
            this.fragmentedSampleSizeBytes += writeStartCode();
            parsableByteArray.getData()[1] = (byte) b4;
        ...
        this.fragmentedSampleSizeBytes += bytesLeft;
        if (z2) {
            this.bufferFlags = getBufferFlagsFromNalType(b4 & Ascii.US);
        }
    }
