...
class TricubicSplineFunction implements TrivariateFunction {
    ...
    public TricubicSplineFunction(double[] dArr) {
        ...
        iArr[1] = 4;
        iArr[0] = 4;
        this.f65a = (double[][][]) Array.newInstance(Double.TYPE, iArr);
        for (int i = 0; i < 4; i++) {
            for (int i2 = 0; i2 < 4; i2++) {
                for (int i3 = 0; i3 < 4; i3++) {
                    this.f65a[i][i2][i3] = dArr[(((i3 * 4) + i2) * 4) + i];
                }
            }
        }
    }
    ...
    public double value(double d, double d2, double d3) throws OutOfRangeException {
        ...
        if (d < 0.0d || d > 1.0d) {
            ...
            for (int i = 0; i < 4; i++) {
                for (int i2 = 0; i2 < 4; i2++) {
                    for (int i3 = 0; i3 < 4; i3++) {
                        d4 += this.f65a[i][i2][i3] * dArr[i] * dArr2[i2] * dArr3[i3];
                    }
                }
            }
            return d4;
